...
const Display = () => {
  ...
  useEffect(() => {
    const updateTasksInFirebase = async () => {
      if (userId) {
        try {
          const userDocRef = doc(db, "users", userId);

          await updateDoc(userDocRef, { tasks });
          console.log("Tasks updated in Firebase successfully", userDocRef);
        } catch (error) {
  ...
  return (
    <div className="center pt-16">
      <div className="flex flex-center ">
        <div className="flex w-fit items-center lg:pl-0 lg:pt-0 xl:pt-0 " onClick={() => navigate("/action")}>
          <p className="text-blue-600 font-bold">Add Task</p>
        </div>
